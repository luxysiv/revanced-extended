...
delete_cache(){
    CACHE_URL="https://api.github.com/repos/$GITHUB_REPOSITORY/actions/caches"

    # Fetching caches
    response=$(wget --quiet --header="Authorization: Bearer $GITHUB_TOKEN" \
                    --header="Accept: application/vnd.github.v3+json" \
                    -O - "$CACHE_URL")
 
    # Checking if the response is successful
    status_code=$(echo "$response" | jq -r '. | select(.actions_caches != null)')

    if [[ -n "$status_code" ]]; then
        caches=$(echo "$response" | jq -r '.actions_caches[] | .id')

        if [[ -n "$caches" ]]; then
            for cache_id in $caches; do
                delete_url="$CACHE_URL/$cache_id"
                delete_response=$(wget --quiet --method=DELETE --header="Authorization: Bearer $GITHUB_TOKEN" \
                                       --header="Accept: application/vnd.github.v3+json" \
                                       --output-document=/dev/null \
                                       --server-response \
                                       "$delete_url" 2>&1 | awk '/^  HTTP/{print $2}')

                if [[ "$delete_response" -eq 204 ]]; then
                    echo "Deleted cache ID: $cache_id"
                else
                    echo "Failed to delete cache ID: $cache_id, status code: $delete_response"
                fi
            done
        else
            echo "No old caches to delete, only one or zero caches found."
        fi
    else
        echo "Failed to fetch caches."
    fi
}
